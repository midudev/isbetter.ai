...
export function statPill(icon: string, value: string, label: string, best = false) {
  ...
  return `
    <div tabindex="0" data-metric-tooltip="${esc(label)}" aria-label="${esc(`${label}: ${value}`)}" class="flex items-center gap-1.5 rounded-md border ${best ? "border-[var(--color-accent)]/40 bg-[var(--color-accent)]/10 text-[var(--color-accent)]" : "border-[var(--color-line)] text-[var(--color-ink-dim)]"} px-2 py-1">
      ...
      ${best ? `<span class="text-[8px] font-medium uppercase tracking-wide">best</span>` : ""}
    </div>`;
}
  ...
  if (viewMode === "preview") {
    ...
    return `
      <div class="relative h-full">
        ...
        <iframe data-preview="${esc(resultKey)}" class="h-full w-full bg-white" sandbox="allow-scripts allow-modals allow-forms" srcdoc="${esc(doc)}" title="Preview generated by ${esc(r.id)}"></iframe>
      </div>`;
  }
  if (viewMode === "code") {
    ...
    return `
      <div class="relative h-full">
        <button data-action="copy" data-model="${esc(resultKey)}" aria-label="Copy generated code" class="absolute right-2 top-2 z-10 flex items-center gap-1 rounded-md border border-[var(--color-line)] bg-[var(--color-panel)]/90 px-2 py-1 text-[10px] text-[var(--color-ink-dim)] backdrop-blur transition-colors hover:text-[var(--color-ink)]">
...
export function providerBadge(p?: string) {
  ...
  return `<span class="inline-flex shrink-0 items-center gap-1 rounded px-1 py-0.5 text-[9px] font-medium uppercase tracking-wide" style="color:${meta.color};background:${meta.color}1a" title="${esc(meta.name)}"><img src="${esc(meta.logo)}" alt="" class="size-3.5 object-contain${mono}">${esc(meta.short)}</span>`;
}
  ...
  return `
    <figure data-timeline-chart data-series="${esc(JSON.stringify(trackingSeries))}" data-max-time="${maxTime}" data-max-tokens="${maxTokens}" data-max-cost="${maxCost}" data-chart-left="${left}" data-chart-right="${right}" data-chart-top="${top}" data-chart-bottom="${bottom}" class="relative" aria-label="Tokens and cost timeline by model">
      ...
      <figcaption class="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--color-line)] px-4 py-2.5">
        ...
        <span class="flex shrink-0 items-center gap-3 text-[9px] uppercase tracking-wider text-[var(--color-ink-faint)]">
...
export function renderBattleInsights(results: SummaryResult[]): string {
  ...
  const insight = (icon: string, label: string, value: string, color: string) => `
    <div class="rounded-lg border border-[var(--color-line)] bg-[var(--color-surface)] p-3">
      <div class="flex items-center gap-1.5 text-[9px] uppercase tracking-wider text-[var(--color-ink-faint)]">${svg(icon, `size-3.5 ${color}`)}${label}</div>
      <div class="mt-1.5 font-mono text-[15px] tabular-nums text-[var(--color-ink)]">${value}</div>
    </div>`;
  return `
    <div class="p-4">
      <div class="mb-3 text-[9px] uppercase tracking-[0.18em] text-[var(--color-ink-faint)]">Battle at a glance</div>
...
export function renderBattleSummary(results: SummaryResult[]): string {
  ...
  return `
    <table class="w-full min-w-[760px] border-collapse text-left text-[11px]">
      <thead class="text-[9px] uppercase tracking-[0.16em] text-[var(--color-ink-faint)]">
        <tr>
        ...
        ${results
          .map((result) => {
            ...
            const badges = (winners.get(result.key) || [])
              .map(
                (label) =>
                  `<span class="rounded bg-[var(--color-accent)]/10 px-1.5 py-0.5 text-[8px] uppercase tracking-wide text-[var(--color-accent)]">${label}</span>`,
              )
              .join("");
            return `
              <tr class="border-t border-[var(--color-line)] ${ok ? "" : "text-red-400/80"}">
                <td class="px-4 py-2.5">
                  <div class="flex min-w-[15rem] items-center gap-2">
                    ${result.concealed ? "" : providerBadge(result.provider)}
                    <span class="max-w-[16rem] truncate text-[var(--color-ink)]"${result.concealed ? "" : ` title="${esc(result.key)}"`}>${esc(result.label)}</span>
                    ${result.cached ? `<span class="rounded border border-[var(--color-line)] px-1 py-0.5 text-[8px] uppercase text-[var(--color-ink-faint)]">cached</span>` : ""}
                    ${badges}
                  </div>
                </td>
                ${
